...
if [ "$(grep 'pattern>%d{' $(find src/main -type f | grep -v '.svn' | egrep '.*log.*(\.xml|\.properties)') | wc -l)" -ne 0 ]; then
  ...
  grep 'pattern>%d{' $(find src/main -type f | grep -v '.svn' | egrep '.*log.*(\.xml|\.properties)')
  exit 1
fi
